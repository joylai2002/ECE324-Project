...
class PSO:
    def __init__(self, n_particles, n_features, n_clusters, bounds, w, c1, c2, max_iter, stop_after_no_improvement=10, device='cuda'):
        ...
        self.bounds = (torch.tensor(bounds[0], device=device).view(1, -1).repeat(self.n_clusters, 1), torch.tensor(bounds[1], device=device).view(1, -1).repeat(self.n_clusters, 1))
        self.w = w
        self.c1 = c1
        self.c2 = c2
    ...
    def init_particles_kmeans(self, data):
        kmeans = MiniBatchKMeans(n_clusters=self.n_clusters, random_state=42)
        kmeans.fit(data)
        kmeans_centroids = torch.tensor(kmeans.cluster_centers_, dtype=torch.float16, device=self.device).half()
        particles = [kmeans_centroids.clone() + torch.randn_like(kmeans_centroids) * 0.1 for _ in range(self.n_particles)]
        return particles
    ...
    def optimize(self, data):
        ...
        particles = self.init_particles_kmeans(data)
        ...
        gbest_position = torch.rand((self.n_clusters, self.n_features), device=self.device)
